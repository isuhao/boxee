...
void CGUIFontTTFGL::Render(FontCoordsIndiced& coords, bool useShadow)
{
  ...
  if(useShadow)
  {
    ...
    glDrawElements( GL_TRIANGLES, 6 * size, GL_UNSIGNED_INT, indices);
  ...
  if(g_advancedSettings.m_bWireFrameMode)
  {
    ...
    glDrawElements( GL_LINE_LOOP, 6 * size, GL_UNSIGNED_INT, indices);
  }
  else
#endif
    glDrawElements( GL_TRIANGLES, 6 * size, GL_UNSIGNED_INT, indices);
...
bool CGUIFontTTFGL::LoadShaders()
{
