...
void BXSourcesManager::CopySourcesList(const BXBoxeeSources& sourcesList)
{
  ...
  if (vectorChanged)
  {
    ...
    CGUIMessage refreshMenuMsg(GUI_MSG_UPDATE, WINDOW_INVALID, GUI_MSG_MEDIA_CHANNELS_UPDATE);
    g_windowManager.SendThreadMessage(refreshMenuMsg,WINDOW_BOXEE_BROWSE_MOVIES);
    g_windowManager.SendThreadMessage(refreshMenuMsg,WINDOW_BOXEE_BROWSE_TVSHOWS);
  }
