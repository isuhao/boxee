...
bool BXTrailerSectionsManager::UpdateTrailerSections(unsigned long executionDelayInMS, bool repeat)
{
  ...
  {
    LOG(LOG_LEVEL_ERROR,"BXTrailerSectionsManager::UpdateTrailers - FAILED to allocate CRequestVideoTrailerSectionsListFromServerTask (genres)");
    retVal &= false;
  }
...
void BXTrailerSectionsManager::SetMovieTrailerSections(const std::vector<TrailerSectionItem>& vecMovieTrailers)
{
  ...
  if (vectorChanged)
  {
    ...
    refreshMenuMsg.SetStringParam("Update Movie Trailers");
...
{
  LOG(LOG_LEVEL_DEBUG,"CRequestVideoTrailersListFromServerTask::DoWork - Enter function (genres)");

  if (!g_application.ShouldConnectToInternet())
  {
    LOG(LOG_LEVEL_DEBUG,"CRequestVideoTrailersListFromServerTask::DoWork - [ShouldConnectToInternet=FALSE] -> Exit function (genres)");
    return;
  }
  ...
  {
    LOG(LOG_LEVEL_ERROR,"CRequestVideoTrailersListFromServerTask::DoWork - FAILED to get genres from [strLink=%s] (genres)",strLink.c_str());
    return;
  }
  ...
  {
    LOG(LOG_LEVEL_ERROR,"CRequestVideoTrailersListFromServerTask::DoWork - could not parse genres. [pRootElement=%p] (genres)",pRootElement);
    return;
  }
