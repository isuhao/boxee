...
bool BXUtils::PerformJSONGetRequest(const std::string& url, Json::Value& response, int& returnCode)
{
  if (url.empty())
  {
    return false;
  ...
  std::string strJson = curl.HttpGetString(strUrl.c_str());
...
{
public:
  JsonRequestBG(const CStdString& url, Json::Value& response, int& returnCode, bool silent=false) : m_url(url), m_response(response), m_returnCode(returnCode)
  {
    m_bJobResult = false;
    m_IsSilent = silent;
  }
...
{
  m_bJobResult = BXUtils::PerformJSONGetRequest(m_url, m_response, m_returnCode);
}

Job_Result BXUtils::PerformJSONGetRequestInBG(const std::string& url, Json::Value& response, int& returnCode, bool silent/*=false*/)
{
  JsonRequestBG* job = new JsonRequestBG(url, response, returnCode, silent);
