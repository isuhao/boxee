...
void BXGenresManager::SetMovieGenres(const std::vector<GenreItem>& movieGenresVec)
{
  ...
  if (vectorChanged)
  {
    ...
    CLog::Log(LOGDEBUG,"BXGenresManager::SetMovieGenres, there was a change in the vector, sending update to the menu (genre update)");

    CGUIMessage refreshMenuMsg(GUI_MSG_UPDATE, WINDOW_INVALID, 0);
    refreshMenuMsg.SetStringParam("Update Movie Genres");
...
void BXGenresManager::SetTvGenres(const std::vector<GenreItem>& tvGenresVec)
{
  ...
  {
    //send message to the menu to update
    CLog::Log(LOGDEBUG,"BXGenresManager::SetTvGenres, there was a change in the vector, sending update to the menu (genre update)");

    CGUIMessage refreshMenuMsg(GUI_MSG_UPDATE, WINDOW_INVALID, 0);
    refreshMenuMsg.SetStringParam("Update TV Genres");
    g_windowManager.SendThreadMessage(refreshMenuMsg,WINDOW_DIALOG_BOXEE_BROWSE_MENU);
  }
