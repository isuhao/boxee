...
bool WatchDog::CCheckInternetConnectionJob::HandleInternetConnectionRestore()
{
  ...
  g_windowManager.SendThreadMessage(refreshMenuMsg,WINDOW_DIALOG_BOXEE_BROWSE_MENU); //ask the menu to rebuild in case genres/channels are empty
...
bool WatchDog::CCheckInternetConnectionJob::RefreshCountry()
{
  ...
  if (strResp.empty())
  {
    CLog::Log(LOGERROR,"WatchDog::CCheckInternetConnectionJob::RefreshCountry - Not handling server response to [url=%s] because it is empty (ftu)",url.c_str());
    return false;
  }

  if(!reader.LoadFromString(strResp))
  {
    CLog::Log(LOGERROR,"WatchDog::CCheckInternetConnectionJob::RefreshCountry  - Not handling server response to [url=%s] because failed to load it to BXXMLDocument (ftu)",url.c_str());
    return false;
  }
  ...
  if(!root)
  {
    CLog::Log(LOGERROR,"WatchDog::CCheckInternetConnectionJob::RefreshCountry - Failed to get root from BXXMLDocument of the ping response (ftu)");
    return false;
  }

  if((strcmp(root->Value(),"boxee") != 0))
  {
    CLog::Log(LOGERROR,"WatchDog::CCheckInternetConnectionJob::RefreshCountry - Failed to parse ping response because the root tag ISN'T <boxee> (ftu)");
    return false;
  }
  ...
  while (pingChildElem)
  {
    if (strcmp(pingChildElem->Value(),"country_code") == 0)
    {
      if (pingChildElem && pingChildElem->FirstChild())
      {
        ...
        CLog::Log(LOGINFO,"WatchDog::CCheckInternetConnectionJob::RefreshCountry - found <country_code> element with value [%s] (tz)", countryCode.c_str());
      ...
      {
        return false;
        CLog::Log(LOGERROR,"WatchDog::CCheckInternetConnectionJob::RefreshCountry  - FAILED to read <country_code> element (tz)");
      }
    }
    pingChildElem = pingChildElem->NextSiblingElement();
  }
