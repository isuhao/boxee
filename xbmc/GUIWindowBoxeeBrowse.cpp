...
void CGUIWindowBoxeeBrowse::ShowItems(CFileItemList& list, bool append)
{
  ...
  m_viewControl.SetCurrentView(m_windowState->GetCurrentView(),m_shouldFocusToPanel);
...
void CGUIWindowBoxeeBrowse::OnWindowLoaded()
{
  ...
  CLog::Log(LOGDEBUG,"CGUIWindowBoxeeBrowse::OnWindowLoaded - going to call InitWindowBrowseMenu (browse)(bm)(bmst)");
  InitWindowBrowseMenu();
}
...
bool CGUIWindowBoxeeBrowse::InitWindowBrowseMenu()
{
  ...
  if (!category.IsEmpty())
  {
    CLog::Log(LOGDEBUG,"CGUIWindowBoxeeBrowse::CGUIWindowBoxeeBrowse - since [category=%s] going to call UpdateWindowBrowseMenu (browse)(bm)",category.c_str());
    ...
    {
      CLog::Log(LOGERROR,"CGUIWindowBoxeeBrowse::CGUIWindowBoxeeBrowse - GUI_MSG_WINDOW_INIT - FAILED to generate BrowseMenu (browse)(bm)");
      return false;
    }
  }
...
bool CGUIWindowBoxeeBrowse::AddButtonItemsToBrowseMenu(const CFileItemList& browseMenu,int browseMenuListControlId/*, int menuLevelToReset*/)
{
