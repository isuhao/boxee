...
#endif
#ifdef HAS_DVB
    else if (strSetting.Equals("ota.status"))
    {
      CGUIButtonControl *pControl = (CGUIButtonControl *)GetControl(GetSetting(strSetting)->GetID());
      pControl->SetEnabled(false);

      if (DVBManager::GetInstance().GetTuners().size() > 0)
      {
        pControl->SetLabel2(g_localizeStrings.Get(58026));
      }
      else
      {
        pControl->SetLabel2(g_localizeStrings.Get(58027));
      }
    }
    else if (strSetting.Equals("ota.device"))
    {
      CGUIButtonControl *pControl = (CGUIButtonControl *)GetControl(GetSetting(strSetting)->GetID());
      pControl->SetEnabled(false);
      ...
      {
        pControl->SetLabel2(g_localizeStrings.Get(58030));
      }
      else
      {
        pControl->SetLabel2(g_localizeStrings.Get(58019));
      }
    }
    else if (strSetting.Equals("ota.location"))
    {
      CGUIButtonControl *pControl = (CGUIButtonControl *)GetControl(GetSetting(strSetting)->GetID());
      pControl->SetEnabled(false);

      if (g_guiSettings.GetString("ota.countrycode") == "")
      {
        pControl->SetLabel2("Unknown");
      }
      else
      {
        CStdString location;
        location.Format("%s", g_guiSettings.GetString("ota.countrycode").c_str());
        pControl->SetLabel2(location);
      }
    }
    else if (strSetting.Equals("ota.rescan"))
    {
      CGUIButtonControl *pControl = (CGUIButtonControl *)GetControl(GetSetting(strSetting)->GetID());
      pControl->SetEnabled(!DVBManager::GetInstance().IsScanning());
...
void CGUIWindowSettingsCategory::OnSettingChanged(CBaseSettingControl *pSettingControl)
{
  ...
  {
    CGUIButtonControl* pControl = (CGUIButtonControl*)GetControl(pSettingControl->GetID());
    if( pControl->IsSelected() && !g_IntelSMDGlobals.CheckCodecHDMIBitstream( CODEC_ID_AC3 ) )
      g_application.m_guiDialogKaiToast.QueueNotification("","",g_localizeStrings.Get(54150),5000);
  ...
  {
    CGUIButtonControl* pControl = (CGUIButtonControl*)GetControl(pSettingControl->GetID());
    if( pControl->IsSelected() && !g_IntelSMDGlobals.CheckCodecHDMIBitstream( CODEC_ID_EAC3 ) )
      g_application.m_guiDialogKaiToast.QueueNotification("","",g_localizeStrings.Get(54151),5000);
  }
  else if( strSetting.Equals("audiooutput.truehdpassthrough") &&
           g_guiSettings.GetInt("audiooutput.mode") == AUDIO_DIGITAL_HDMI )
  {
    CGUIButtonControl* pControl = (CGUIButtonControl*)GetControl(pSettingControl->GetID());
    if( pControl->IsSelected() && !g_IntelSMDGlobals.CheckCodecHDMIBitstream( CODEC_ID_TRUEHD ) )
      g_application.m_guiDialogKaiToast.QueueNotification("","",g_localizeStrings.Get(54152),5000);
  }
  else if( strSetting.Equals("audiooutput.dtspassthrough") &&
           g_guiSettings.GetInt("audiooutput.mode") == AUDIO_DIGITAL_HDMI )
  {
    CGUIButtonControl* pControl = (CGUIButtonControl*)GetControl(pSettingControl->GetID());
    if( pControl->IsSelected() && !g_IntelSMDGlobals.CheckCodecHDMIBitstream( CODEC_ID_DTS ) )
      g_application.m_guiDialogKaiToast.QueueNotification("","",g_localizeStrings.Get(54153),5000);
  ...
  {
    CGUIButtonControl* pControl = (CGUIButtonControl*)GetControl(pSettingControl->GetID());
    if( pControl->IsSelected() && !g_IntelSMDGlobals.CheckCodecHDMIBitstream( SMD_CODEC_DTSHD ) )
      g_application.m_guiDialogKaiToast.QueueNotification("","",g_localizeStrings.Get(54154),5000);
  }
...
//end Boxee
  ...
  else if (strSetting.Equals("ota.rescan"))
  {
    ...
    if (CGUIWindowBoxeeMain::RunOnBoardingWizardIfNeeded())
    {
  ...
  {
    g_guiSettings.SetBool("ota.run", false);
    g_guiSettings.SetBool("ota.scanned", false);
    if (CGUIWindowBoxeeMain::RunOnBoardingWizardIfNeeded())
    {
      g_windowManager.ActivateWindow(WINDOW_BOXEE_LIVETV);
...
void CGUIWindowSettingsCategory::UpdateOtaStatus()
{
  ...
  if (!bOtaSectionVisible || (refreshCounter % 60 != 0))
  {
    return;
  }

  CBaseSettingControl *otaStatusSetting = GetSetting("ota.rescan");
  ...
  {
    int p = DVBManager::GetInstance().GetScanPercent();
    if (p >= 100)
      p = 99;
    str.Format(g_localizeStrings.Get(58054).c_str(), p);
  }
  else if (DVBManager::GetInstance().GetChannels().Size() == 0 || !g_guiSettings.GetBool("ota.scanned"))
  {
    str = g_localizeStrings.Get(58053);
  }
  else
  {
