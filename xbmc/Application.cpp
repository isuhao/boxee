...
HRESULT CApplication::Create(HWND hWnd)
{
  ...
  g_Windowing.EnableDepthTest(true);
  ...
  case GUI_MSG_PLAYLISTPLAYER_STOPPED:
    {
      ...
      if (!IsPlayingVideo() && g_windowManager.GetActiveWindow() == WINDOW_FULLSCREEN_VIDEO && message.GetMessage() != GUI_MSG_PLAYBACK_ENDED)
      {
        g_windowManager.PreviousWindow();
      }
...
void CApplication::BoxeePostLoginInitializations()
{
 ...
 /*
  ...
  if(!m_BoxeeSocialUtilsManager.InitBoxeeSocialUtilsManager())
  {
...
void CApplication::BoxeeUserLogoutAction()
{
  ...
  m_BoxeeSocialUtilsManager.ResetBoxeeSocialUtilsManager();
...
bool CApplication::ShouldConnectToInternet()
{
  return (IsConnectedToInternet() && !IsInScreenSaver() && !((IsPlayingVideo() && (GetCurrentPlayer() != EPC_FLASHPLAYER))|| IsPlayingAudio()));
}
