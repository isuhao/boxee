...
CAddVideoBG::CAddVideoBG(BOXEE::BXMetadata& resolvedVideoMetadata, CVideoFileContext& videoFileContext, CFileItemList& videoParts) :
...
bool CAddVideoBG::AddVideo()
{
  ...
  CSendConfirmationBG* pJob = new CSendConfirmationBG(m_videoFileContext);
 ...
 */
CGUIDialogBoxeeManualResolve::CGUIDialogBoxeeManualResolve() : CGUIDialog(WINDOW_DIALOG_BOXEE_MANUAL_RESOLVE, "boxee_manual_resolve.xml"), m_resovedVideoMetadata(MEDIA_ITEM_TYPE_VIDEO)
{
  m_bConfirmed = false;
}
...
bool CGUIDialogBoxeeManualResolve::IdentifyWithNFO()
{
  ...
  else
  {
    ...
    if( m_unresolvedVideoItem->IsUPnP() &&
          NULL != m_unresolvedVideoItem->m_strTitle )
      m_videoFileContext.Load( _P(m_unresolvedVideoItem->m_strTitle) );
    else
      ...
      for (int i = 0; i < episodeItems.Size(); i++)
      {
        ...
        if (currentEpisode->IsVideo())
        {
          ...
          if (context.iSeason == iSavedSeason && context.iEpisode > 0)
          {
            ...
            if (bResult)
            {
              CAddVideoBG* pAddVideoJob = new CAddVideoBG(resolvedVideoMetadata, context, m_videoParts);
              CUtil::RunInBG(pAddVideoJob);
            }
            else
            {
              // if the job has been cancelled, we need to wait for thread exit.
              // otherwise resolvedVideoMetadata will be deleted while the thread is running
              if (pJob->m_IsCanceled)
              {
                CThread *pThread = pJob->pParent;
                if (pThread)
                  pThread->WaitForThreadExit(DEFAULT_THREAD_WAIT_TIMEOUT);
              }
              CLog::Log(LOGERROR, "CGUIDialogBoxeeManualResolve::OnMessage, unable to get details for item, (manual)");
            }
          }
