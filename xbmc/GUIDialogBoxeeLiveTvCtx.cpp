...
void CGUIDialogBoxeeLiveTvCtx::HandleClickedChannel()
{
  ...
  if (pSelectedItem->GetPropertyBOOL("hasepg") && !pSelectedItem->GetPropertyBOOL("isnow"))
  {
    ...
    if (pInfo->IsRequestTune() && CGUIWindowBoxeeLiveTv::SwitchChannel(pSelectedItem->GetPropertyInt("channel-id")))
    {
      Close();
    }
  ...
  if (!pSelectedItem->GetPropertyBOOL("isactive") && CGUIWindowBoxeeLiveTv::SwitchChannel(pSelectedItem->GetPropertyInt("channel-id")))
  {
    Close();
    return;
  }
