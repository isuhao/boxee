...
#define AUTO_CLOSE_PIC_CTX_DLG              3000
...
void CBackgroundPicLoader::Process()
{
  ...
  bool loadFirstPic = true;
  
  while (!m_bStop)
  {
    // loop around forever, waiting for the app to call LoadPic
    if (WaitForSingleObject(m_loadPic, 1000) == WAIT_OBJECT_0)
    {
      if (m_pCallback)
      {
        unsigned int start = CTimeUtils::GetTimeMS();

        CLog::Log(LOGDEBUG,"CBackgroundPicLoader::Process - START load picture [m_strFileName=%s] (loadp)",m_strFileName.c_str());
        CGUIDialogProgress * progress = NULL;
        
        if(loadFirstPic)
        {
          if (!CUtil::IsHD(m_strFileName))
          {
            progress = (CGUIDialogProgress *)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
            if (progress)
            {
              progress->SetHeading(51003);
              progress->SetLine(0, "");
              progress->SetLine(1, "");
              progress->SetLine(2, g_localizeStrings.Get(51003));
        ...
        CLog::Log(LOGDEBUG,"CBackgroundPicLoader::Process - END load picture [m_strFileName=%s] (loadp)",m_strFileName.c_str());
        if (progress)
          progress->Close();
...
void CBackgroundPicLoader::LoadPic(int iPic, int iSlideNumber, const CStdString &strFileName, const CStdString &strContentType, const int maxWidth, const int maxHeight)
{
  m_iPic = iPic;
...
bool CGUIWindowSlideShow::IsPaused() const
{
  return m_bPause;
}
...
void CGUIWindowSlideShow::FreeResources()
{
  ...
  if (m_pBackgroundLoader)
  {
    ...
    int numOfSleepCounter = 0;
    bool IsBackgroundLoaderInLoading = m_pBackgroundLoader->IsLoading();

    CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::FreeResources - START waitting for BackgroundLoader thread to close [IsBackgroundLoaderInLoading=%d][numOfSleepCounter=%d] (blt)",IsBackgroundLoaderInLoading,numOfSleepCounter);

    while (IsBackgroundLoaderInLoading)
    {
      Sleep(10);
      
      IsBackgroundLoaderInLoading = m_pBackgroundLoader->IsLoading();
      
      numOfSleepCounter++;      
      if(numOfSleepCounter > 10)
      {
        // In case we called Sleep(10) more then 10 times -> Break
        
        CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::FreeResources - Going to break from while loop because [numOfSleepCounter=%d>10]. [IsBackgroundLoaderInLoading=%d] (blt)",numOfSleepCounter,IsBackgroundLoaderInLoading);

        break;
      }
    }

    CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::FreeResources - END waitting for BackgroundLoader thread to close [IsBackgroundLoaderInLoading=%d][numOfSleepCounter=%d] (blt)",IsBackgroundLoaderInLoading,numOfSleepCounter);

    // stop the thread
    CLog::Log(LOGDEBUG,"Stopping BackgroundLoader thread");
    m_pBackgroundLoader->Detach(true);
    m_pBackgroundLoader = NULL;
  }
...
bool CGUIWindowSlideShow::InScreensaver() const
{
...
void CGUIWindowSlideShow::Render()
{
  ...
  { // we have an error when loading either the current or next picture
    ...
    { // No.  Not much we can do here.  If we're in a slideshow, we mayaswell move on to the next picture
      ...
      if (bSlideShow)
      {
        CLog::Log(LOGDEBUG, "Error loading the current image %s", m_slides->Get(m_iCurrentSlide)->m_strPath.c_str()); // changed to DEBUG to avoid spamming the log
  ...
  { 
    ...
    GetCheckedSize((float)g_settings.m_ResInfo[m_Resolution].iWidth * zoomamount[m_iZoomFactor - 1],
                    ...
                    maxWidth, maxHeight);
    m_pBackgroundLoader->LoadPic(m_iCurrentPic, m_iCurrentSlide, m_slides->Get(m_iCurrentSlide)->m_strPath, m_slides->Get(m_iCurrentSlide)->GetContentType(), maxWidth, maxHeight);
  }
  ...
  if (m_bReloadImage)
  {
    ...
    { // reload the image if we need to
      ...
      m_pBackgroundLoader->LoadPic(m_iCurrentPic, m_iCurrentSlide, m_slides->Get(m_iCurrentSlide)->m_strPath, m_slides->Get(m_iCurrentSlide)->GetContentType(), width, height);
    }
  }
  else
  {
    ...
    { // load the next image
      ...
      GetCheckedSize((float)g_settings.m_ResInfo[m_Resolution].iWidth * zoomamount[m_iZoomFactor - 1],
                     (float)g_settings.m_ResInfo[m_Resolution].iHeight * zoomamount[m_iZoomFactor - 1],
                     maxWidth, maxHeight);
      m_pBackgroundLoader->LoadPic(1 - m_iCurrentPic, m_iNextSlide, m_slides->Get(m_iNextSlide)->m_strPath, m_slides->Get(m_iNextSlide)->GetContentType(), maxWidth, maxHeight);
    }
  }
...
bool CGUIWindowSlideShow::OnAction(const CAction &action)
{
 ...
 case ACTION_SHOW_INFO:
    {      
//Boxee
      CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::OnAction, NEWUI, open picture context");
      CGUIDialogBoxeePictureCtx* pDlgInfo = (CGUIDialogBoxeePictureCtx*)g_windowManager.GetWindow(WINDOW_DIALOG_BOXEE_PICTURE_CTX);
      if (pDlgInfo)
      {
        CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::OnAction, NEWUI, opening context dialog");
...
//end Boxee
 ...
 case ACTION_PARENT_DIR:
  ...
  case ACTION_SHOW_CODEC:
    {
      CLog::Log(LOGDEBUG,"CGUIWindowSlideShow::OnAction, NEWUI, open picture info");
      CGUIDialogPictureInfo *pictureInfo = (CGUIDialogPictureInfo *)g_windowManager.GetWindow(WINDOW_DIALOG_PICTURE_INFO);
      if (pictureInfo)
      {
        // no need to set the picture here, it's done in CGUIDialogPictureInfo::Render()
        pictureInfo->DoModal();
      }
    }
    break;
  ...
  case ACTION_STOP:
    ...
    // TODO - Make it generic when Boxee start using a real screensaver (and not slideshow)
    g_application.SetInSlideshowScreensaver(false);
...
bool CGUIWindowSlideShow::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_START_SLIDESHOW:
    {
      CFileItemList* pSlideList = (CFileItemList*)message.GetPointer();
...
void CGUIWindowSlideShow::OnLoadPic(int iPic, int iSlideNumber, CBaseTexture* pTexture, int iOriginalWidth, int iOriginalHeight, bool bFullSize)
{
  if (pTexture)
  {
    ...
    else
    {
      ...
      m_Image[iPic].SetTexture(iSlideNumber, pTexture, dispEffect, transEffect);
...
void CGUIWindowSlideShow::RunSlideShow(const CFileItemList & slides, bool bRandom /* = false */, bool bNotRandom /* = false */, const CStdString& pictureToStart /*= ""*/, bool bStartInPause /*= false*/)
{
  Reset();
  m_slides->Append(slides);

  RunSlideShow(bRandom, bNotRandom, pictureToStart, bStartInPause);
}

void CGUIWindowSlideShow::RunSlideShow(const CStdString &strPath, bool bRecursive /* = false */, bool bRandom /* = false */, bool bNotRandom /* = false */, const CStdString& pictureToStart /*= ""*/, bool bStartInPause /*= false*/)
{
...
void CGUIWindowSlideShow::ToggleShowSlideInfo()
{
  m_showSlideInfo = !m_showSlideInfo;
}

bool CGUIWindowSlideShow::ShowSlideInfo()
{
  return m_showSlideInfo;
}

bool CGUIWindowSlideShow::IsBackgroundLoaderLoading(bool localPath)
{
  bool isBackgroundLoaderLoading = false;
  
  if(m_pBackgroundLoader)
  {
    if(localPath == CUtil::IsHD(m_pBackgroundLoader->GetFileName()))
    {
      isBackgroundLoaderLoading = m_pBackgroundLoader->IsLoading();      
    }
  }
  
  return isBackgroundLoaderLoading;
}
