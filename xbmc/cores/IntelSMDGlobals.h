...
public:
  ...
  ismd_dev_t              GetAudioDevice(bool timed = true) { return timed ? m_audioDev : m_audioDevNonTimed; }
  ismd_port_handle_t      GetAudioInput(bool timed = true)  { return timed ? m_audio_inputPort : m_audio_inputPortNonTimed; }
  ismd_audio_output_t     GetAudioOutput(int out) { return (out >= 0 && out <= 3 ? m_audio_output[out] : -1); }
  bool                    GetAudioOutputAdded(int out) { return (out >= 0 && out <= 3 ? m_audio_output_device_added[out] : false); }
  int                     GetAudioOutputCount()   { return m_audio_output_count; }

  bool  AddAudioOutput(int audio_device, ismd_audio_output_config_t device_config);
  bool  RemoveAudioOutput();
  bool  DisableAudioOutput();
  bool  EnableAudioOutput();
  bool  EnableAudioInput(bool timed);
  bool  DisableAudioInput(bool timed);

  bool SetAudioDeviceBaseTime(ismd_time_t time);
  ...
  bool CreateAudioDevice(bool timed);
  bool DeleteAudioProcessor();
  bool DeleteAudioDevice(bool timed);

  bool SetAudioDeviceState(ismd_dev_state_t state, bool timed = true);
  ...
  ismd_dev_state_t GetAudioDeviceState(bool timed = true);

  ismd_codec_type_t GetVideoCodec() { return m_video_codec; }

  bool FlushAudioDevice(bool timed = true);
  ...
  bool CheckCodecHDMIBitstream( int Codec );
  bool CheckCodecHWDecode( int Codec );

  void SetCodecHDMIBitstream( int Codec, bool val );
  void SetCodecHWDecode( int Codec );
...
protected:
  ...
  ismd_dev_t              m_audioDev;
  ismd_dev_t              m_audioDevNonTimed;
  ismd_port_handle_t      m_audio_inputPort;
  ismd_port_handle_t      m_audio_inputPortNonTimed;

  ismd_audio_output_t     m_audio_output[3];
  bool                    m_audio_output_device_added[3];
  int                     m_audio_output_count;
  ...
  // Format support
  int   m_HDMIBitstreamCodecs[6][2];
  int   m_HWDecodeCodecs[HW_AUDIO_DECODING_NUM][2];
