...
bool EpgServerLoader::PullSocial()
{
  ...
  int returnCode;
  if (!BOXEE::BXUtils::PerformJSONGetRequest(strUrl, jsonValue, returnCode))
  {
...
//    socialInfo.friends.push_back(f);
    ...
    if (numOfFriendsWatching && moreWatching == 1)
      socialInfo.label += " and[CR]one other person is watching";
    else if (numOfFriendsWatching && moreWatching > 1)
      socialInfo.label.Format("%s and[CR]%d others are watching", socialInfo.label.c_str(), moreWatching);
...
{
  CLog::Log(LOGDEBUG, "Pull epg start");
