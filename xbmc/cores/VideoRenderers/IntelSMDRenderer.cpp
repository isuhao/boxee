...
bool CIntelSMDRenderer::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, CRect &rect)
{
  ...
  CLog::Log(LOGNONE, "CIntelSMDRenderer configure overscan values left %d top %d right %d bottom %d", left, top, right, bottom);
  ...
  if (m_bUsingSMDecoder)
    printf("Video rendering using SMD decoder\n");
  else if (m_bNullRendering)
    printf("Video rendering using NULL renderer\n");
  else
    printf("Video rendering using software decoder\n");
...
{
  printf("CIntelSMDRenderer::ConfigureGDLPlane\n");
...
int CIntelSMDRenderer::ConfigureVideoFilters()
{
  ...
  printf("CIntelSMDRenderer Setting deringing_filter %d\n", deringing_filter);
  printf("CIntelSMDRenderer Setting gaussian filter %d\n", gaussian_filter);
