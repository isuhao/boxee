...
CLinuxRendererGL::CLinuxRendererGL()
{
  ...
  for (int i = 0; i < NUM_BUFFERS; i++)
    m_eventTexturesDone[i] = new CEvent(false);
...
bool CLinuxRendererGL::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, CRect &browserRect)
{
  m_sourceWidth = width;
  ...
  if (m_externImagePbo)
    glDeleteBuffersARB(1, &m_externImagePbo);
  m_externImagePbo = 0;

  if ((flags & CONF_FLAGS_EXTERN_IMAGE) && glewIsSupported("GL_ARB_pixel_buffer_object") && glGenBuffersARB )
  {
    CLog::Log(LOGDEBUG,"CLinuxRendererGL::Configure - using PBO for RGB picture rendering");
    glGenBuffersARB(1, &m_externImagePbo);
    if (m_externImagePbo)
    {
      glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, m_externImagePbo);
...
int CLinuxRendererGL::GetImage(YV12Image *image, double pts, int source, bool readonly)
{
  if (!image) return -1;
...
void CLinuxRendererGL::RenderUpdate(bool clear, DWORD flags, DWORD alpha)
{
  ...
  glPushMatrix();
  glLoadMatrixf((float *)matProjection->m);
...
{
  bool pixelRatioChanged    = (g_stSettings.m_fPixelRatio > 1.001f || g_stSettings.m_fPixelRatio < 0.999f) !=
                              (m_pixelRatio > 1.001f || m_pixelRatio < 0.999f);
  bool nonLinStretchChanged = false;
  if (m_nonLinStretchGui != g_stSettings.m_bNonLinStretch || pixelRatioChanged)
  {
    m_nonLinStretchGui   = g_stSettings.m_bNonLinStretch;
    m_pixelRatio         = g_stSettings.m_fPixelRatio;
    m_reloadShaders      = 1;
  ...
  if (m_scalingMethodGui == g_stSettings.m_currentVideoSettings.m_ScalingMethod && !nonLinStretchChanged)
    return;
...
{
  if (m_iFlags & CONF_FLAGS_EXTERN_IMAGE)
    return;
...
void CLinuxRendererGL::RenderSinglePass(int index, int field)
{
  ...
  if (g_application.m_pPlayer && g_application.m_pPlayer->IsInMenu())
    ...
    m_pYUVShader->SetNonLinStretch(pow(g_stSettings.m_fPixelRatio, g_advancedSettings.m_videoNonLinStretchRatio));
...
void CLinuxRendererGL::RenderMultiPass(int index, int field)
{
  ...
  if (m_pVideoFilterShader)
  {
    ...
    if (g_application.m_pPlayer && g_application.m_pPlayer->IsInMenu())
      ...
      m_pVideoFilterShader->SetNonLinStretch(pow(g_stSettings.m_fPixelRatio, g_advancedSettings.m_videoNonLinStretchRatio));
...
void CLinuxRendererGL::RenderSoftware(int index, int field)
{
  ...
  glVertex4f(0.0, 0.0, 0, 1.0f );

  glTexCoord2f(planes[0].rect.x2, planes[0].rect.y1);
  glVertex4f(destRect.Width(), 0.0, 0, 1.0f);

  glTexCoord2f(planes[0].rect.x2, planes[0].rect.y2);
  glVertex4f(destRect.Width(), destRect.Height(), 0, 1.0f);

  glTexCoord2f(planes[0].rect.x1, planes[0].rect.y2);
  glVertex4f(0.0, destRect.Height(), 0, 1.0f);
...
void CLinuxRendererGL::SetRGB32Image(char *image, int nHeight, int nWidth, int nPitch)
{
  if (!image && !m_externImagePbo)
    return; // use the prev pointer. NULL image means - nothing new, use old pointer.
  ...
  m_externImage = (BYTE*)image;
  m_renderMethod = RENDER_SW;

  YUVBUFFER& buf =  m_buffers[m_iYV12RenderBuffer];
  buf.image.flags = IMAGE_FLAG_READY;
  buf.image.height = nHeight;
  buf.image.width = nWidth;
  buf.image.stride[0] = nPitch;
  buf.image.plane[0] = NULL;
...
void CLinuxRendererGL::RenderRGBImage(int index)
{
  YUVBUFFER& buf    =  m_buffers[index];
  YV12Image* im     = &buf.image;
  YUVFIELDS& fields =  buf.fields;
  ...
  if (m_externImagePbo)
  {
    ...
    if (m_externImage)
    {
      glBufferDataARB(GL_PIXEL_UNPACK_BUFFER_ARB, (im->stride[0]) * (im->height), 0, GL_STREAM_DRAW_ARB);
      GLubyte* ptr = (GLubyte*)glMapBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, GL_WRITE_ONLY_ARB);
      if(ptr)
      {
        memcpy(ptr, m_externImage, (im->stride[0]) * (im->height));
        glUnmapBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB);
      }
    }
    glTexSubImage2D(m_textureTarget, 0, 0, 0, im->width, im->height, GL_BGRA, GL_UNSIGNED_BYTE, 0);
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, 0);
  }
  else
    glTexSubImage2D(m_textureTarget, 0, 0, 0, im->width, im->height, GL_BGRA, GL_UNSIGNED_BYTE, m_externImage);
  ...
  RenderSoftware(index,FIELD_FULL);
