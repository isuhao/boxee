  ...
  else
  {
    ...
    switch(requestedMethod)
    {
      ...
      case RENDER_METHOD_D3D_PS:
        ...
        if (m_deviceCaps.PixelShaderVersion >= D3DPS_VERSION(2, 0) && !(m_flags & (CONF_FLAGS_EXTERN_IMAGE | CONF_FLAGS_FULLSCREEN)))
        {
        ...
        {
          CLog::Log(LOGNOTICE, "D3D: graphics adapter does not support Pixel Shaders 2.0 OR rendering external/rgb image (browser mode)");
        }
...
bool CWinRenderer::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, CRect browserRect)
{
...
int CWinRenderer::GetImage(YV12Image *image, double pts, int source, bool readonly)
{
...
void CWinRenderer::SelectPSVideoFilter()
{
  ...
  if (m_scalingMethod == VS_SCALINGMETHOD_AUTO)
  {
    ...
    bool scaleFps = m_fps < (videoAutoScaleMaxFps + 0.01f);
...
void CWinRenderer::UpdatePSVideoFilter()
{
  ...
  if (m_bUseHQScaler)
  {
    ...
    {
      SAFE_RELEASE(m_scalerShader);
      CLog::Log(LOGNOTICE, "Video Renderering: Failed to init video scaler, falling back to bilinear scaling.");
      m_bUseHQScaler = false;
    }
  }
...
{
  if (m_scalingMethodGui == g_stSettings.m_currentVideoSettings.m_ScalingMethod && m_bFilterInitialized)
    return;
...
void CWinRenderer::RenderSW()
{
  ...
  {
    if(m_flags & (CONF_FLAGS_EXTERN_IMAGE | CONF_FLAGS_FULLSCREEN)) //browser mode...
	{
		RenderRGBImage();
	}
...
void CWinRenderer::SetRGB32Image(char *image, int nHeight, int nWidth, int nPitch)
{
  if(image == NULL)
    return;
  ...
  int newBufferSize = nPitch*nHeight;
  if(m_rgbBufferSize != newBufferSize)
    m_srcSizeChanged = true;

  m_rgbBufferSize = newBufferSize;
...
void CWinRenderer::RenderRGBImage()
{
  ...
  if(m_texture.Get() == NULL || m_srcSizeChanged)
  {	  
    CLog::Log(LOGNOTICE, "%s(%d) creating new videro texture. size = %d x %d\n", __FUNCTION__, __LINE__, m_sourceWidth, m_sourceHeight);	
	m_texture.Release();
	bool res = m_texture.Create(m_sourceWidth, m_sourceHeight, 1, g_Windowing.DefaultD3DUsage(), D3DFMT_A8R8G8B8, g_Windowing.DefaultD3DPool());
	if(!res)
	{
	  CLog::Log(LOGERROR, __FUNCTION__" - error creating texture");
	  return;
	}
	m_srcSizeChanged = false;
  }
  ...
  if(m_bRGBImageSet)
  {
	if (m_texture.LockRect( 0, &lr, NULL, D3DLOCK_DISCARD ))
	{
	  ...
	  unsigned char *src = m_rgbBuffer;
	  unsigned int dstPitch = lr.Pitch;
	  unsigned int srcPitch = m_sourceWidth * 4;
	  unsigned int minPitch = std::min(srcPitch, dstPitch);

	  unsigned int rows = m_sourceHeight;
  ...
  pD3DDevice->SetTexture(0, m_texture.Get());
  ...
  CRect rectVid = m_destRect;
  if (m_flags & (CONF_FLAGS_EXTERN_IMAGE | CONF_FLAGS_EXTERN_IMAGE_WITH_VIDEO))
	  rectVid = m_browserRect;

  CUSTOMVERTEX verts[4] = 
  {
    {
      rectVid.x1, rectVid.y1, 0.0f, 0xffffffff, (m_sourceRect.x1) / m_sourceWidth, (m_sourceRect.y1) / m_sourceHeight,
    },
    {
      rectVid.x2, rectVid.y1, 0.0f, 0xffffffff, (m_sourceRect.x2) / m_sourceWidth, (m_sourceRect.y1) / m_sourceHeight,
    },
    {
      rectVid.x2, rectVid.y2, 0.0f, 0xffffffff, (m_sourceRect.x2) / m_sourceWidth, (m_sourceRect.y2) / m_sourceHeight,
    },
    {
      rectVid.x1, rectVid.y2, 0.0f, 0xffffffff, (m_sourceRect.x1) / m_sourceWidth, (m_sourceRect.y2) / m_sourceHeight,
    }
  };
...
void CWinRenderer::ScaleFixedPipeline()
{
  ...
  bool cbcontrol          = (g_stSettings.m_currentVideoSettings.m_Contrast != 50.0f || g_stSettings.m_currentVideoSettings.m_Brightness != 50.0f);
  unsigned int contrast   = (unsigned int)(g_stSettings.m_currentVideoSettings.m_Contrast *.01f * 255.0f); // we have to divide by two here/multiply by two later
  unsigned int brightness = (unsigned int)(g_stSettings.m_currentVideoSettings.m_Brightness * .01f * 255.0f);
...
void CWinRenderer::Stage1()
{
  if (!m_bUseHQScaler)
  {
      m_colorShader->Render(m_sourceRect, m_destRect,
                            g_stSettings.m_currentVideoSettings.m_Contrast,
                            g_stSettings.m_currentVideoSettings.m_Brightness,
                            m_flags,
