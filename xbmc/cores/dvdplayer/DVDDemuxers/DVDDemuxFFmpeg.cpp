      ...
      if (pPacket)
      {
        ...
        if (pPacket->dts != DVD_NOPTS_VALUE && stream->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
          if (m_lastVideoDTS && abs(((int) (m_dtsCorrection + pPacket->dts)) - m_lastVideoDTS) > DVD_SEC_TO_TIME(5))
          {
            ...
            printf("m_dtsCorrection %.2f m_lastDTS %.2f pPacket->dts %.2f\n", m_dtsCorrection, m_lastVideoDTS, pPacket->dts);
            m_dtsCorrection = m_lastVideoDTS - pPacket->dts;
            printf("CDVDDemuxFFmpeg::gap detected, changing dts correction to %.2f\n", m_dtsCorrection);
          }
        ...
        if(m_bMatroska && stream->codec && stream->codec->profile == FF_PROFILE_DTS_HD_MA)
        {
...
void CDVDDemuxFFmpeg::AudioStreamChanged()
{
  m_lastAudioDTS = 0;
}
