...
bool CDVDAudioCodecSMD::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options)
{
  ...
  printf("CDVDAudioCodecSMD::Open successful. codec %d bSupportsHWDecode %d bSupportsPassthrough %d bSupportedSampleRate %d\n",
        hints.codec, bSupportsHWDecode, bSupportsPassthrough, bSupportedSampleRate);
...
void CDVDAudioCodecSMD::GetStreamInfo()
{
  ...
  audio_device = g_IntelSMDGlobals.GetAudioDevice(true);

  if(audio_processor == -1 || audio_device == -1)
    return;

  result = ismd_audio_input_get_stream_info(
      g_IntelSMDGlobals.GetAudioProcessor(),
      g_IntelSMDGlobals.GetAudioDevice(true),
      &audio_info);
