...
#include "StackedMovieBackSeekHandler.h"
...
#define STREAM_OPEN_OK 1
#define STREAM_OPEN_NOT_ENOUGH_INFO 2
#define STREAM_OPEN_NOT_SUPPORTED 3
#define STREAM_OPEN_ERROR 4
...
int CSelectionStreams::IndexOf(StreamType type, CDVDPlayer& p)
{
  if (p.m_pInputStream && p.m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    ...
    {      
      if(STREAM_SOURCE_MASK(p.m_CurrentSubtitle.source) == STREAM_SOURCE_TEXT)
      {
        source = p.m_CurrentSubtitle.source;
        id = p.m_CurrentSubtitle.id;
      }
      else
      {
        id = ((CDVDInputStreamNavigator*)p.m_pInputStream)->GetActiveSubtitleStream();

        SelectionStream s = Get(STREAM_SUBTITLE, 0);
        if (STREAM_SOURCE_MASK(s.source) == STREAM_SOURCE_TEXT)
        {
          ++id;
        }
      }
    }
    return IndexOf(type, source, id);
  }
...
void CSelectionStreams::Update(CDVDInputStream* input, CDVDDemux* demuxer)
{
  if(input && input->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    ...
    for(int i=0;i<count;i++)
    {
      ...
      s.name     = nav->GetAudioStreamName(i);
    ...
    for(int i=0;i<count;i++)
    {
      ...
      s.id       = i + hasExtSubs;
      s.language = nav->GetSubtitleStreamLanguage(i);
      s.name     = nav->GetSubtitleStreamName(i);
      s.flags    = CDemuxStream::FLAG_NONE;
      s.filename = filename;
      Update(s);
    }
  ...
  else if(demuxer)
  {
    ...
    for(int i=0;i<count;i++)
    {
      ...
      if(stream->type == STREAM_AUDIO && s.name.empty())
      {
        std::string type;
  ...
  m_waitForInitialPlay(true)
{
  ...
  m_caching = false;
  m_seeking = false;
  m_seekrecovery = false;
  m_cachesuppress = 0;
  m_lastError = IPLAYER_SUCCESS;

  m_pDlgCache = NULL;
  m_pausedCaching = false;
...
bool CDVDPlayer::OpenFile(const CFileItem& file, const CPlayerOptions &options)
{
  try
  {
    ...
    if (!file.IsDVD() && !file.GetPropertyBOOL("DisableBoxeeUI"))
    {
      if (file.IsInternetStream())
        m_pDlgCache = new CDlgCache(0, g_localizeStrings.Get(10214), file.GetLabel());
      else
        m_pDlgCache = new CDlgCache(3000, "", file.GetLabel());
    }
    ...
    bool bDisablePtsCorrection = file.GetPropertyBOOL("DisablePtsCorrection");
    m_dvdPlayerAudio.DisablePtsCorrection(bDisablePtsCorrection);
    m_dvdPlayerVideo.DisablePtsCorrection(bDisablePtsCorrection);

    CGUIDialogProgress *dlg = (CGUIDialogProgress*)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
    bool bWaitForStart = !file.GetPropertyBOOL("FastPlayerStart");
    while (bWaitForStart && WaitForSingleObject(m_hReadyEvent, 30) != WAIT_OBJECT_0)
    {
    ...
    {
//    printf("\n************************************ %s::%s(%d)\tcalling m_callback.OnPlayBackStarted()\n", __FILE__, __FUNCTION__, __LINE__);
      m_callback.OnPlayBackStarted();
    }

#ifdef HAS_EMBEDDED
    if(options.fullscreen) {
//    printf("\n************************************ %s::%s(%d)\toptions.fullscreen = %d - switching to full screen\n", __FILE__, __FUNCTION__, __LINE__, options.fullscreen);
      g_application.getApplicationMessenger().SwitchToFullscreen();
//  } else {
//    printf("\n************************************ %s::%s(%d)\toptions.fullscreen = %d - NOT switching to full screen\n", __FILE__, __FUNCTION__, __LINE__, options.fullscreen);
	}
#endif
...
{
  DWORD ret = WaitForSingleObject(m_hReadyEvent, 0);
  return !m_bStop && (ret==WAIT_OBJECT_0);
}
...
bool CDVDPlayer::OpenInputStream()
{
  ...
  if(m_item.IsPlayableFolder())
  {
    if(m_item.GetPropertyBOOL("isBlurayFolder") == true)
    {
      filename = m_filename;
    ...
    else if(m_item.GetPropertyBOOL("isDVDFolder") == true)
    {
      m_filename = m_filename + "VIDEO_TS/VIDEO_TS.IFO";
    }
    else 
    {
      CLog::Log(LOGERROR, "CDVDPlayer::OpenInputStream - unknown playable folder [%s]", m_filename.c_str());
    }
  }
      ...
      &&  !m_pInputStream->IsStreamType(DVDSTREAM_TYPE_HTSP))
  {
    ...
    CDVDFactorySubtitle::GetSubtitles(filenames, m_filename, m_content);
  ...
  m_internetStream = m_item.IsInternetStream() || m_item.IsInternetPlayList();
...
bool CDVDPlayer::OpenDemuxStream()
{
  ...
  {
    int attempts = 10;
    while(!m_bStop && !m_bAbortRequest && attempts-- > 0)
    {
...
bool CDVDPlayer::OpenDefaultStreams()
{
  ...
  for(int i = 0;i<count && !valid;i++)
  {
    ...
    reason = OpenVideoStream(s.id, s.source);
    if(reason == STREAM_OPEN_OK)
      valid = true;
  }
  if(!valid)
  {
...
// on the box we might encounter unsupported formats.
// if we don't support the video playback then we quit here
#ifdef HAS_EMBEDDED
  if(!valid && reason == STREAM_OPEN_NOT_SUPPORTED)
  {
    printf("CDVDPlayer::OpenDefaultStreams Format no supported\n");
    return false;
  }
#endif
  ...
  if(!m_PlayerOptions.video_only)
  {
    ...
    CStdString user_lang = g_settings.m_subtitleLangToCodeMap[g_guiSettings.GetString("audioplayback.preferredlanguage")];

    // we only open default track if the user didn't select a language
    if(!valid && user_lang.Equals(CStdString("def"))
    ...
    // if we didn't restore an audio stream then try to be smart about which stream to use
    // we will process each stream twice - once in an attempt to hw bitstream, and another
    // as software decode. we do native language first then non native language
    // missing a language string is assumed to be native or non locale specific
    // the expensive part of this is the case insensitive language compares... ugh
    if( !valid )
    {
      // Try bitstream native language first
      for(int i = 0; i<count && !valid; i++)
      {
        SelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, i);
        if(user_lang.Equals(CStdString(s.language)))
          if(OpenAudioStream(s.id, s.source, true))
            valid = true;
      }

      // Next try native language non bitstream
      for(int i = 0; i<count && !valid; i++)
      {
        SelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, i);
        if(user_lang.Equals(CStdString(s.language)))
          if(OpenAudioStream(s.id, s.source, false))
            valid = true;
      }
      // Next try non-native language bitstream
      for(int i = 0; i<count && !valid; i++)
      {
        SelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, i);
        if(!user_lang.Equals(CStdString(s.language)))
      ...
      if (!valid && m_SelectionStreams.Get(STREAM_AUDIO, CDemuxStream::FLAG_DEFAULT, st))
      {
        if (OpenAudioStream(st.id, st.source))
          ...
          CLog::Log(LOGWARNING, "%s - failed to open default stream 2 (%d)",
              __FUNCTION__, st.id);
      }
  ...
  {
    SelectionStream st;
    st = m_SelectionStreams.Get(STREAM_VIDEO, 0);
    m_CurrentVideo.id = st.id;

    st = m_SelectionStreams.Get(STREAM_AUDIO, 0);
    m_CurrentAudio.id = st.id;
  }
#endif
...
void CDVDPlayer::Process()
{
  ...
  if (!OpenInputStream())
  {
    m_bAbortRequest = true;
    m_has_err = true;
    m_err = "Open file failed";
    m_lastError = IPLAYER_FILE_OPEN_ERROR;
    CLog::Log(LOGNOTICE, "OpenInputStream failed!");
    return;
  }
  ...
  if(!OpenDemuxStream())
  {
    m_bAbortRequest = true;
    m_has_err = true;
    m_lastError = IPLAYER_FORMAT_NOT_SUPPORTED;
    return;
  }
  ...
  m_dvdPlayerVideo.SetHtml5EmbeddedVideo(m_item.GetPropertyBOOL("HTML5Media"));


#ifdef HAS_EMBEDDED
  bool defaultStreamOpened = OpenDefaultStreams();
  if(!defaultStreamOpened)
  {
    m_bAbortRequest = true;
    m_has_err = true;
    m_lastError = IPLAYER_FORMAT_NOT_SUPPORTED;
  ...
  double subsStartPts = DVD_NOPTS_VALUE;
  int starttime = 0;
  ...
  if( m_PlayerOptions.starttime > 0 || m_PlayerOptions.startpercent > 0)
  {
    ...
    {
      __int64 playerStartTime = (__int64) (((float) m_pDemuxer->GetStreamLength()) *  m_PlayerOptions.startpercent);
    ...
    if(m_pSubtitleDemuxer)
    {
      if(m_pSubtitleDemuxer->SeekTime(starttime, false, &subsStartPts))
  ...
  // we are done initializing now, set the ready event
  ...
  // don't do timing based queue on bluray, it's not accurate enough
  if(m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY))
  {
    m_dvdPlayerAudio.m_messageQueue.SetMaxTimeSize(0);
    m_dvdPlayerVideo.m_messageQueue.SetMaxTimeSize(0);
  }

  if(m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV))
  {
    m_dvdPlayerAudio.m_messageQueue.SetMaxTimeSize(1);
    m_dvdPlayerVideo.m_messageQueue.SetMaxTimeSize(1);
  }
  ...
  while (!m_bAbortRequest)
  {
    ...
    // if we were in seek recovery and the queues are filled past 15%,
    // then reset our seek state.
    // note we can't stop caching here; the queues were empty so the players were stalled,
    // and they will simply trigger another recache; caching has to stop before
    // we exit seek recovery, and we need to process a few packets first
    if( m_seekrecovery && 15 < GetCacheLevel() )
    {
      m_seekrecovery = false;
      m_cachesuppress = 0;

      CSingleLock lock(m_StateSection);
      if( m_State.seek_pending_time && !m_State.seek_depth)
      {
        //m_State.seek_pending_time = 0;
      }
    }
    ...
    // if we're in seek recovery, once we get to 10% we stop caching
    if( m_seekrecovery && 10 < GetCacheLevel() )
    {
      // avoid an infinite caching loop - once we have suppressed the caching loop a couple of times,
      // we should let it do a full rebuffer
      if( m_caching && m_cachesuppress < 2)
      {
        m_cachesuppress++;
        SetCaching(false);
      }
    ...
    // if we are in seek recovery and the cache level is > 5% , stop caching
  //  if( m_caching && m_seekrecovery && GetCacheLevel() > 10 )
  //    SetCaching(false);
    ...
    // Pass passthorugh packets to video player for optional user data
    if(pPacket && pPacket->flags & DEMUX_PACKET_FLAG_PASSTHROUGH)
    {
      CDVDDemuxUtils::FreeDemuxPacket(pPacket);
      //m_dvdPlayerVideo.SendMessage(new CDVDMsgDemuxerPacket(pPacket, false));
      continue;
    }
    ...
    try
    {
      ...
      {
#ifdef HAS_EMBEDDED
        int reason = OpenVideoStream(pStream->iId, pStream->source);
        if(reason == STREAM_OPEN_NOT_SUPPORTED)
        {
          m_bAbortRequest = true;
          m_has_err = true;
          m_lastError = IPLAYER_FORMAT_NOT_SUPPORTED;
...
void CDVDPlayer::ProcessAudioData(CDemuxStream* pStream, DemuxPacket* pPacket)
{
  ...
  {
    /* if we hit a codec or extra data change then reopen the stream
     * for channel count, sample rate, etc we leave as-is
     */
    CDVDStreamInfo nfo = CDVDStreamInfo(*pStream, true);
    if( m_CurrentAudio.hint != nfo )
    {
      CLog::Log(LOGDEBUG, "%s - reopening audio stream due to hint change (cod %d/%d ch %d/%d)",
          __FUNCTION__, m_CurrentAudio.hint.codec, nfo.codec, m_CurrentAudio.hint.channels, nfo.channels);
      ...
      if( !OpenAudioStream( pPacket->iStreamId, pStream->source, true ) )
        OpenAudioStream( pPacket->iStreamId, pStream->source, false );

      // DVDPlayerAudio sets itself to playing when you call CloseAudioStream,
      // so ensure we are in sync here.
      if( m_caching ) m_dvdPlayerAudio.SetSpeed( DVD_PLAYSPEED_PAUSE );
      else            m_dvdPlayerAudio.SetSpeed(m_playSpeed);
    }
  ...
  {
    if (!m_pInputStream || m_pInputStream->GetReadAhead() == 0 || (m_item.GetPropertyBOOL("HTML5Media") && m_waitForInitialPlay))
    {
      m_waitForInitialPlay = false;
      CLog::Log(LOGDEBUG, "CDVDPlayer::ProcessAudioData caching...\n");
      SetCaching(true);
    }
  }
...
bool CDVDPlayer::CheckStartCaching(CCurrentStream& current)
{
  ...
  int audioLevels = 100 * m_dvdPlayerAudio.m_messageQueue.GetDataSize() / (float)m_dvdPlayerAudio.m_messageQueue.GetMaxDataSize();
  int videoLevels = 100 * m_dvdPlayerVideo.m_messageQueue.GetDataSize() / (float)m_dvdPlayerVideo.m_messageQueue.GetMaxDataSize();

  //printf("audio_lvl %d video_lvl %d passthrough %d\n", audioLevels, videoLevels, m_dvdPlayerAudio.IsPassthrough());
  ...
  // We can be not-stalled but still be starving in seek recovery.
  if( m_seekrecovery && (audioLevels < 1 && videoLevels < 1) )
  {
    return true;
  }
...
#ifdef HAS_INTEL_SMD
   if((m_dvdPlayerAudio.IsPassthrough() || m_dvdPlayerAudio.IsHWDecode()) && videoLevels > 0)
     return false;
#endif
...
bool CDVDPlayer::CheckPlayerInit(CCurrentStream& current, unsigned int source)
{
  ...
  if (current.inited == false && current.dts != DVD_NOPTS_VALUE)
  {
    ...
    if(starttime > 0)
    {
      // don't send a delay if the gap is too long, or if we are recovering from a seek and this is a video
      // packet. ideally we show video asap and sync later
      ...
      else if( DVDPLAYER_VIDEO == source )
...
{
  CLog::Log(LOGDEBUG, "CDVDPlayer::OnExit\n");
  ...
  if(m_bAbortRequest)
  {
    ...
    g_IntelSMDGlobals.SetAudioDeviceState(ISMD_DEV_STATE_STOP, true);
  }
#endif
  ...
  try
  {
    ...
    if (m_pInputStream)
    {
      m_has_err = m_pInputStream->HasFileError(&m_err);
  ...
  {
    if (m_bAbortRequest) {
//    printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackStopped()\n", __FILE__, __FUNCTION__, __LINE__);
      m_callback.OnPlayBackStopped();
    } else {
//    printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackEnded(has_err = %d, m_err = %s) \n", __FILE__, __FUNCTION__, __LINE__, m_has_err, m_err.c_str());
      m_callback.OnPlayBackEnded(m_has_err, m_err);
    }
  }
...
#ifdef HAS_EMBEDDED
  // clear inode cache - should improve performance
  ...
  while (ret == MSGQ_OK)
  {
    ...
    try
    {
      if (pMsg->IsType(CDVDMsg::PLAYER_SEEK))
      {
        ...
        {
          CLog::Log(LOGDEBUG, "demuxer seek to: %4.2f, success", start);
          //printf("demuxer seek success. Time requested %.2f  Start seek %.2f\n", time / 1000.0, start / 1000000.0f);
          if(m_pSubtitleDemuxer)
          {
          ...
          m_seekrecovery = true;
          {
            // lock
            CSingleLock lock(m_StateSection);
            if( m_State.seek_depth )           
              m_State.seek_depth--;    // we popped from the queue
            // unlock
          }
...
//#ifdef HAS_INTEL_SMD
        m_clock.Discontinuity(start);
//#endif
      }
      else if (pMsg->IsType(CDVDMsg::PLAYER_SEEK_CHAPTER))
      {
        CLog::Log(LOGDEBUG, "CDVDPlayer::HandleMessages PLAYER_SEEK_CHAPTER\n");
        CPlayerSeek m_pause(this);
        ...
        {
          if( DVD_NOPTS_VALUE != start )
          {
            CSingleLock lock(m_StateSection);
            m_State.seek_pending_time = llrint(start) / 1000;
          }

          FlushBuffers(false);
          m_seekrecovery = true;

//        printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackSeekChapter()\n", __FILE__, __FUNCTION__, __LINE__);
      ...
      {
        CLog::Log(LOGDEBUG, "CDVDPlayer::HandleMessages DEMUXER_RESET\n");
      ...
      {
        printf("CDVDPlayer::HandleMessages PLAYER_SET_AUDIOSTREAM\n");
        CDVDMsgPlayerSetAudioStream* pMsg2 = (CDVDMsgPlayerSetAudioStream*)pMsg;

        // notify demuxer about the change
        if(m_pDemuxer)
          m_pDemuxer->AudioStreamChanged();
        ...
        if(st.source != STREAM_SOURCE_NONE)
        {
          ...
          {
            // if we use timed audio/video, wait for the audio to drain before switching audio streams
            // this stops us from empty audio/full video rebuffer conditions where we drop audio frames
            // due to timestamp gaps
            bool bDrain = m_dvdPlayerVideo.IsTimed();
            CloseAudioStream( bDrain );
      ...
      {
        printf("CDVDPlayer::HandleMessages PLAYER_SET_SUBTITLESTREAM\n");
        ...
        if(st.source != STREAM_SOURCE_NONE)
        {
          if(st.source == STREAM_SOURCE_NAV && m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
          {
            ...
            CDVDInputStreamNavigator* pStream = (CDVDInputStreamNavigator*)m_pInputStream;
            if(pStream->SetActiveSubtitleStream(iStream))
            {
              if(iStream == st.id)
...
#ifdef HAS_INTEL_SMD
            CACHE_QUEUE_TYPE cached_data = m_sub_cache.GetCachedPackets(st.source, st.id);
            for(CACHE_QUEUE_TYPE::iterator iter=cached_data.begin(); iter != cached_data.end(); ++iter)
            {
              ProcessSubData((*iter)->GetStream(), (*iter)->GetPacket());
            }
#endif

            ChooseAndOpenSubtitleStream(st.id, st.source);
          }
        }
      }
      else if (pMsg->IsType(CDVDMsg::PLAYER_SET_SUBTITLESTREAM_VISIBLE))
      {
        printf("CDVDPlayer::HandleMessages PLAYER_SET_SUBTITLESTREAM_VISIBLE\n");
      ...
      {
        printf("CDVDPlayer::HandleMessages PLAYER_SET_STATE\n");
        CPlayerSeek m_pause(this);
      ...
      {
        printf("CDVDPlayer::HandleMessages PLAYER_SET_RECORD\n");
      ...
      {
        //printf("CDVDPlayer::HandleMessages GENERAL_FLUSH\n");
        FlushBuffers(false);
      }
      else if (pMsg->IsType(CDVDMsg::PLAYER_SETSPEED))
      {
        //printf("CDVDPlayer::HandleMessages PLAYER_SETSPEED\n");
        int speed = static_cast<CDVDMsgInt*>(pMsg)->m_value;

        //printf("DVDPlayer speed %d\n", speed);
        ...
        if (speed != DVD_PLAYSPEED_PAUSE)
        {
          if (m_playSpeed != DVD_PLAYSPEED_PAUSE)
          {
//          printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackSpeedChanged()\n", __FILE__, __FUNCTION__, __LINE__);
...
void CDVDPlayer::SetCaching(bool enabled)
{
  /*
  if(enabled)
    printf("\nCDVDPlayer::Start Caching\n");
  else
    printf("\nCDVDPlayer::End Caching\n");
  */

  if(m_caching == enabled) {
//    printf("\nCDVDPlayer.SetCaching() - nothing to change\n");
    return;
  }

  if(enabled)
  {
    if(m_item.GetPropertyBOOL("HTML5Media"))
    {
      /**
       * while playing HTML5 media, when the player starts caching it should
       * pause and not resume automatically when caching has finished.
       */
      m_clock.SetSpeed(DVD_PLAYSPEED_PAUSE);
      m_dvdPlayerAudio.SetSpeed(DVD_PLAYSPEED_PAUSE);
      m_dvdPlayerVideo.SetSpeed(DVD_PLAYSPEED_PAUSE);
      m_pausedCaching = true; //player is paused due to caching
      m_callback.OnPlayBackPaused(); //notify FVP
    }
    else if (!m_item.GetPropertyBOOL("SilentCaching"))
    {
      CLog::Log(LOGDEBUG, "CDVDPlayer::SetCaching - started caching%s", m_seekrecovery?" (seek recovery)":"");
      //printf("CDVDPlayer::SetCaching - started caching%s\n", m_seekrecovery?" (seek recovery)":"");
      m_clock.SetSpeed(DVD_PLAYSPEED_PAUSE);
      m_dvdPlayerAudio.SetSpeed(DVD_PLAYSPEED_PAUSE);
      m_dvdPlayerVideo.SetSpeed(DVD_PLAYSPEED_PAUSE);
    }
  ...
  else
  {
    if(m_item.GetPropertyBOOL("HTML5Media"))
    {
      //HTML5Media playback - NOT RESUMING
    }
    else if (!m_item.GetPropertyBOOL("SilentCaching"))
    {
      CLog::Log(LOGDEBUG, "CDVDPlayer::SetCaching - stopped caching");
      //printf("CDVDPlayer::SetCaching - stopped caching\n");
...
void CDVDPlayer::Pause()
{
  ...
  if(m_playSpeed != DVD_PLAYSPEED_PAUSE && m_caching && !m_item.GetPropertyBOOL("SilentCaching") && !m_item.GetPropertyBOOL("HTML5Media"))
  {
    SetCaching(false);
    return;
  }
  ...
  if(m_item.GetPropertyBOOL("HTML5Media"))
  {
    if(m_pausedCaching) //player is paused since it was caching
    {
      if(!m_caching) //not caching now
      {
        m_clock.SetSpeed(m_playSpeed);
        m_dvdPlayerAudio.SetSpeed(m_playSpeed);
        m_dvdPlayerVideo.SetSpeed(m_playSpeed);
//        printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackResumed()\n", __FILE__, __FUNCTION__, __LINE__);
...
//    printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackPaused()\n", __FILE__, __FUNCTION__, __LINE__);
...
bool CDVDPlayer::HasLiveTV() const
{
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV))
    return true;
...
bool CDVDPlayer::CanSeek()
{
  ...
  {
    return ((CDVDInputStreamPlaylist*)m_pInputStream)->CanSeek();
  }
  return GetTotalTime() > 0;
}

bool CDVDPlayer::CanSeekToTime()
{
  return CanSeek();
}
...
bool CDVDPlayer::CanPause() const
{
  if(m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_PLAYLIST))
  {
    return ((CDVDInputStreamPlaylist*)m_pInputStream)->CanPause();
  }
...
void CDVDPlayer::Seek(bool bPlus, bool bLargeStep)
{
  ...
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_PLAYLIST))
  {
    CAction action;
    if (bPlus)
      action.id = bLargeStep?ACTION_BIG_STEP_FORWARD:ACTION_STEP_FORWARD;
    else
      action.id = bLargeStep?ACTION_BIG_STEP_BACK:ACTION_STEP_BACK;
    OnAction(action);
    return ;
  }
#endif
  ...
  {
    if (m_pInputStream && (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY) || m_pInputStream->IsStreamType(DVDSTREAM_TYPE_PLAYLIST)))
    {
      CLog::Log(LOGDEBUG, "CDVDPlayer::Seek - seeking large step instead of next chapter");
    }
    else
    {
      if(bPlus)
  ...
  __int64 seek, seekDelta;
  if (g_advancedSettings.m_videoUseTimeSeeking && GetTotalTime() > 2*g_advancedSettings.m_videoTimeSeekForwardBig)
  {
    if (bLargeStep)
      seekDelta = bPlus ? g_advancedSettings.m_videoTimeSeekForwardBig : g_advancedSettings.m_videoTimeSeekBackwardBig;
    else
      seekDelta = bPlus ? g_advancedSettings.m_videoTimeSeekForward : g_advancedSettings.m_videoTimeSeekBackward;
    seekDelta *= 1000;
  }
  else
  {
    ...
    seekDelta = (__int64)(GetTotalTimeInMsec() * percent / 100);
  }

  {
    __int64 dur = GetTotalTimeInMsec();

    // lock
    CSingleLock lock(m_StateSection);

    __int64 currTime = GetTime();
    if(m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_PLAYLIST))
    {
      currTime = ((CDVDInputStreamPlaylist*)m_pInputStream)->GetTime() * 1000;
    }

    seek = (m_State.seek_pending_time > 0) ? m_State.seek_pending_time : currTime;

    // Don't move the timeline backwards
    if( seek + seekDelta < 0 )
      seek = 0;
    else if( dur > 0 && seek + seekDelta > dur )
    {
      // Don't let the timeline seek past the end of the content; stop 10s before the end
      if( seek < (dur - 10000) )
        seek = dur - 10000;
      else
      {
    ...
    m_State.seek_depth++;
    m_State.seek_pending_time = seek;
  ...
  /*
   * check if the backward seek will jump to previous part of stacked movie
   */
  if (g_application.PlayingStackedMovie() && !bPlus && seek < 0)
  {
    CStackedMovieBackSeekHandler h(seek);
    g_application.getApplicationMessenger().ExecuteOnMainThread((IGUIThreadTask*)&h);
  }
  else
  {
    // flush seek messages before issuing a new one
    m_messenger.Flush(CDVDMsg::PLAYER_SEEK);
  ...
  if (seek < 0) seek = 0;
//printf("***************** %s::%s(%d)\tcalling m_callback.OnPlayBackSeek()\n", __FILE__, __FUNCTION__, __LINE__);
...
{
  CSingleLock lock(m_StateSection);
  if(m_pDemuxer && m_pDemuxer->IsAudioPassthrough())
    strAudioInfo.Format("%s", m_State.demux_audio.c_str());
  else
    strAudioInfo.Format("D( %s ) \nP( %s )", m_State.demux_audio.c_str()
...
{
  CSingleLock lock(m_StateSection);
  if(m_pDemuxer && m_pDemuxer->IsVideoPassthrough())
      strVideoInfo.Format("%s", m_State.demux_video.c_str());
  else
    strVideoInfo.Format("D( %s ) \nP( %s )", m_State.demux_video.c_str()
...
float CDVDPlayer::GetPercentageWithCache()
{
  if (!m_pInputStream || m_pInputStream->GetLength() == 0)
    return 0.0f;

  return (float)(m_pInputStream->GetPosition() + m_pInputStream->GetReadAhead())  * 100.0 / (float)m_pInputStream->GetLength() + 1;
}

int CDVDPlayer::GetPositionWithCache() //seconds
{

	double ff = m_dvdPlayerVideo.GetTimeWithCache();
	...
	return (int)(ff / 1000000);
}

void CDVDPlayer::SetVolume(long nVolume)
{
#ifdef HAS_INTEL_SMD
  if(m_pDemuxer->IsAudioPassthrough())
  {
    IAudioRenderer* audio = m_pDemuxer->GetAudioDevice();
    if(audio)
      audio->SetCurrentVolume(nVolume);
  } else
#endif
...
void CDVDPlayer::GetSubtitleLang(int iStream, CStdString &strStreamLang)
{
  strStreamLang = "";
  SelectionStream& s = m_SelectionStreams.Get(STREAM_SUBTITLE, iStream);
...
bool CDVDPlayer::GetSubtitleForced()
{
  return m_dvdPlayerVideo.GetForcedSub();
}

void CDVDPlayer::SetSubtitleForced(bool bVisible)
{
  m_dvdPlayerVideo.SetForcedSub(bVisible);
}
...
void CDVDPlayer::GetAudioStreamLang(int iStream, CStdString &strStreamLang)
{
  strStreamLang = "";
  SelectionStream& s = m_SelectionStreams.Get(STREAM_AUDIO, iStream);
  if (s.language.length() > 0)
...
__int64 CDVDPlayer::GetTime()
{
  ...
  time = llrint(m_State.time + DVD_TIME_TO_MSEC(offset));

//printf("\n***************** %s::%s(%d)\ttime = %lld\n\n", __FILE__, __FUNCTION__, __LINE__, time);
  return time;

  /*
  if( m_State.seek_pending_time && time < m_State.seek_pending_time)
    return m_State.seek_pending_time;
  else
  {
    m_State.seek_pending_time = 0;
    return time;
...
{
  // Don't lock here as this is an atomic read. 
  //CSingleLock lock(m_StateSection);
  return llrint(m_State.time_total);
}

int CDVDPlayer::GetStartTime()
{
  if (m_pDemuxer)
    return m_pDemuxer->GetStartTime();
  return 0;
}
...
bool CDVDPlayer::OpenAudioStream(int iStream, int source, bool force_hardware)
{
  CLog::Log(LOGNOTICE, "Opening audio stream: %i source: %i forcehw: %c", iStream, source, force_hardware?'t':'f');
  ...
  if (!HasVideo())
  {
    if (m_item.IsMMS() || m_item.GetPropertyBOOL("IsLiveStream") || m_item.GetPropertyBOOL("custom:IsLiveStream"))
    {
      m_dvdPlayerAudio.m_messageQueue.SetMaxDataSize(5 * 16 * 1024);
    }
  }

  if (m_item.HasProperty("MaxAudioQueueSize"))
    m_dvdPlayerAudio.m_messageQueue.SetMaxDataSize(m_item.GetPropertyInt("MaxAudioQueueSize"));
  
  CDVDStreamInfo hint(*pStream, true);

  hint.bitstream = force_hardware;
...
int CDVDPlayer::OpenVideoStream(int iStream, int source)
{
  ...
  if (m_item.HasProperty("MaxVideoQueueSize"))
    m_dvdPlayerVideo.m_messageQueue.SetMaxDataSize(m_item.GetPropertyInt("MaxVideoQueueSize"));
  else if (m_item.IsMMS() || m_item.GetPropertyBOOL("IsLiveStream"))
    m_dvdPlayerVideo.m_messageQueue.SetMaxDataSize(2 * 256 * 1024);
  ...
  // for liveTV, do not allow zero width & height
  if( m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_TV) )
  {
    if(!hint.width && !hint.height)
    {
      return STREAM_OPEN_NOT_ENOUGH_INFO;
    }
  }
      ...
      || m_CurrentVideo.hint != hint)
  {
    if (!m_dvdPlayerVideo.OpenStream(hint))
    {
      ...
      pStream->disabled = true;
      pStream->SetDiscard(AVDISCARD_ALL);
      if(hint.codec == CODEC_ID_NONE)
        return STREAM_OPEN_NOT_ENOUGH_INFO;
      else
        return STREAM_OPEN_NOT_SUPPORTED;
    }
  }
  else
  ...
  return STREAM_OPEN_OK;
}

bool CDVDPlayer::ChooseAndOpenSubtitleStream(int iStream, int source)
{
  int count = m_SelectionStreams.Count(STREAM_SUBTITLE);
  bool bForced = false;
  bool bDefault = false;
  bool bSubsOn = false;
  CStdString strLangCode = "";
  SelectionStream selectStream;
  selectStream.id = -1;

  strLangCode = g_settings.m_subtitleLangToCodeMap[g_guiSettings.GetString("subtitles.preferredlanguage")];
  bSubsOn = g_stSettings.m_currentVideoSettings.m_SubtitleOn;

  printf("Language selected %s sub on %d\n", strLangCode.c_str(), bSubsOn);

  // first try to open function params
  if (OpenSubtitleStream(iStream, source))
  {
    printf("Open function params. stream %d bForced %d\n", iStream, bForced);
    goto subs_done;
  }

  // Next try to open saved settings
  if(g_stSettings.m_currentVideoSettings.m_SubtitleStream >= 0
      && g_stSettings.m_currentVideoSettings.m_SubtitleStream < count)
  {
    selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, g_stSettings.m_currentVideoSettings.m_SubtitleStream);
    if(OpenSubtitleStream(selectStream.id, selectStream.source))
    {
      printf("Previously selected sub %d\n", g_stSettings.m_currentVideoSettings.m_SubtitleStream);
      goto subs_done;
    }
    else
  ...
  // Next look for external subtitles
  for (int i = 0; i < count; i++)
  {
    selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, i);
    if (STREAM_SOURCE_MASK(selectStream.source) == STREAM_SOURCE_TEXT)
    {
      if (OpenSubtitleStream(selectStream.id, selectStream.source))
      {
        printf("Found external text sub. Set to select\n");
        goto subs_done;
      }
    }
  }

  // If selected language is set to default, open default subs, if any
  if (strLangCode.Equals(CStdString("def")))
  {
    if (m_SelectionStreams.Get(STREAM_SUBTITLE, CDemuxStream::FLAG_DEFAULT, selectStream))
    {
      if (OpenSubtitleStream(selectStream.id, selectStream.source))
      {
        printf("Found default language match to def\n");
        goto subs_done;
      }
    }
  }
  // if there is a language selection, try to find the matching sub.
  // if subs are on, then the first language match non forced
  // if subs are off, then the first language match forced
  for(int i = 0; i < count; i++)
  {
    selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, i);
    bForced = selectStream.flags & CDemuxStream::FLAG_FORCED;
    bDefault = selectStream.flags & CDemuxStream::FLAG_DEFAULT;

    // Check if we can match the preferred language to the subtitle language
    // We also need to check if the subtitles are on. If not we only show forced
    if(strLangCode.Equals(CStdString(selectStream.language)))
    {
      if(bSubsOn != bForced)
      {
        if(OpenSubtitleStream(selectStream.id, selectStream.source))
        {
          printf("Found matching language %s to %s\n", strLangCode.c_str(), selectStream.language.c_str());
          goto subs_done;
        }
      }
    }
  ...
  // On PGS subs, the forced and non forced are on the same stream
  // So just check the language
  for (int i = 0; i < count; i++)
  {
    selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, i);
    if (strLangCode.Equals(CStdString(selectStream.language)))
    {
      if (OpenSubtitleStream(selectStream.id, selectStream.source))
      {
        printf("Found matching language2 %s to %s\n", strLangCode.c_str(), selectStream.language.c_str());
        goto subs_done;
      }
    }
  }

  // nothing yet... Try to open default stream anyway
  if (m_SelectionStreams.Get(STREAM_SUBTITLE, CDemuxStream::FLAG_DEFAULT, selectStream))
  {
    if (OpenSubtitleStream(selectStream.id, selectStream.source))
    {
      printf("Found default stream\n");
      goto subs_done;
    }
  }

  // Just try to open the first available stream
  for (int j = 0; j < count; j++)
  {
    selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, j);
    if (OpenSubtitleStream(selectStream.id, selectStream.source))
      goto subs_done;
  }


  if (selectStream.id == -1)
  {
    printf("No matching sub found\n");
    CloseSubtitleStream(false);
    return false;
  }
  ...
  int index = m_SelectionStreams.IndexOf(STREAM_SUBTITLE, m_CurrentSubtitle.source, m_CurrentSubtitle.id);
  selectStream = m_SelectionStreams.Get(STREAM_SUBTITLE, index);
  bForced = selectStream.flags & CDemuxStream::FLAG_FORCED;
  printf("Selected sub forced flag %d\n", bForced);
  m_dvdPlayerVideo.SetForcedSub(bForced);

  if (bSubsOn && !m_PlayerOptions.video_only)
    m_dvdPlayerVideo.EnableSubtitle(true);
  else
    m_dvdPlayerVideo.EnableSubtitle(false);

  printf("DVDPlayerVideo subs are %d\n", m_dvdPlayerVideo.IsSubtitleEnabled());
...
bool CDVDPlayer::CloseVideoStream(bool bWaitForBuffers)
{
  ...
  //printf("CDVDPlayer::CloseVideoStream ok\n");

  return true;
}
bool CDVDPlayer::RestartSubtitleStream()
{
  if (!CloseSubtitleStream(false))
  {
    return false;
  }

  return ChooseAndOpenSubtitleStream();
}
...
{
  //printf("CDVDPlayer::FlushBuffers %d\n", queued);
...
#define THREAD_ACTION(action) \
  ...
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    ...
    int eventId = action.id;
    if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
    {
      if (eventId == ACTION_BIG_STEP_FORWARD)
        eventId = ACTION_NEXT_ITEM;
      else if (eventId == ACTION_BIG_STEP_BACK)
        eventId = ACTION_PREV_ITEM;
    }
  ...
  if (dynamic_cast<CDVDInputStream::IChannel*>(m_pInputStream))
  {
    ...
    case ACTION_PAGE_UP:
    case ACTION_MOVE_UP:
      m_messenger.Flush(CDVDMsg::PLAYER_CHANNEL_NEXT);
    ...
    case ACTION_MOVE_DOWN:
      m_messenger.Flush(CDVDMsg::PLAYER_CHANNEL_PREV);
    ...
    {
      // Offset from key codes back to button number
      int channel = (int) action.amount1;
      m_messenger.Flush(CDVDMsg::PLAYER_CHANNEL_SELECT);
  ...
  case ACTION_SHOW_VIDEOMENU:
    if (GetChapterCount() > 0 && m_pDemuxer)
    {
      std::vector<DemuxChapterInfo> chapters;
      int selectedChapter;

      m_pDemuxer->GetChaptersInfo(chapters);

      // found out where are we in the chapters selection
      int nSelection = -1;
      long currentSeconds = GetTime() / 1000;
      for(size_t i = 0; i < chapters.size(); i++)
      {
        DemuxChapterInfo info = chapters[i];
        if(info.startSecond < currentSeconds && info.endSecond > currentSeconds)
        {
          nSelection = i;
          break;
        }
      ...
      if (CGUIDialogBoxeeChapters::Show(chapters, selectedChapter, nSelection))
      {
         SeekChapter(selectedChapter);
      }
    }
    break;
  }
...
bool CDVDPlayer::HasMenu()
{
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
    return true;
  else if(m_pInputStream && GetChapterCount() > 0)
    return true;
  else
    return false;
}
...
int CDVDPlayer::GetStreamPlaylistTimecode()
{
  int timecode = -1;
  if(IsPlayingStreamPlaylist())
  {
    timecode = ((CDVDInputStreamPlaylist*)m_pInputStream)->GetCurrentTimecode();
  }

  return timecode;
}

iplayer_error CDVDPlayer::GetError()
{
  return m_lastError;
}

CStdString CDVDPlayer::GetErrorString()
{
  return m_err;
}
...
bool CDVDPlayer::OnAppMessage(const CStdString& strHandler, const CStdString& strParam)
{
  bool bHandled = false;

  if(strHandler == "mlb:chapters" && m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_PLAYLIST))
  {
    CDVDInputStreamPlaylist* pStream = static_cast<CDVDInputStreamPlaylist*>(m_pInputStream);

    bHandled = pStream->SetChapters(strParam);   
  }

  return bHandled;
}

void CDVDPlayer::GetVideoCacheLevel(unsigned int& cur, unsigned int& max)
{
  cur = m_dvdPlayerVideo.m_messageQueue.GetLevel();
  max = 100;
}

void CDVDPlayer::GetAudioCacheLevel(unsigned int& cur, unsigned int& max)
{
  cur = m_dvdPlayerAudio.m_messageQueue.GetLevel();
  max = 100;
}
