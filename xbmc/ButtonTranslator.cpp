...
#define REMOTEMAPTAG "irssmap"
  ...
  // run through our window groups
  TiXmlNode* pRemote = pRoot->FirstChild("remote");
  while (pRemote)
  {
    ...
    pRemote = pRemote->NextSibling("remote");
  }
...
void CButtonTranslator::MapRemote(TiXmlNode *pRemote, const char* szDevice)
{
  ...
  while (pButton)
  {
    ...
    {
      CStdString strKey = pButton->FirstChild()->Value();
      const char *strAttrLong = pButton->Attribute("long");
      if (strAttrLong && strcmpi(strAttrLong,"true") == 0)
        strKey += "-long";
      buttons[strKey] = pButton->Value();
	  CLog::Log(LOGDEBUG, "CButtonTranslator::MapRemote, mapped key, key = %s, value = %s", strKey.c_str(), pButton->Value());
    }
...
{
  map<CStdString, lircButtonMap>::iterator it = lircRemotesMap.begin();
  while (it != lircRemotesMap.end())
  {
    if (it->first.Left(10) == "/dev/input")
      arrDevs.push_back(it->first);
    it++;
  }
}
#endif
...
bool CButtonTranslator::NeedsLircLongClick(const char* szDevice, const char *szButton)
{
  ...
  // Find the button
  lircButtonMap::iterator it2 = (*it).second.find(CStdString(szButton) + "-long");
  return (it2 != (*it).second.end());
}

WORD CButtonTranslator::TranslateLircLongClick(const char* szDevice, const char *szButton)
{
  // Find the device
  map<CStdString, lircButtonMap>::iterator it = lircRemotesMap.find(szDevice);
  if (it == lircRemotesMap.end())
...
int CButtonTranslator::GetActionCode(int window, const CKey &key, CStdString &strAction)
{
  ...
  XBMCMod mod = key.GetModifiers();
  map<int, buttonMap>::iterator it = translatorMap.find(window);
  if (it == translatorMap.end())
    return 0;
  buttonMap::iterator it2 = (*it).second.find(std::pair<uint32_t, XBMCMod>(code, mod));
  int action = 0;
...
void CButtonTranslator::MapAction(uint32_t buttonCode, const char *szAction, buttonMap &map, const char* modifier)
{
  int action = ACTION_NONE;
  if (!TranslateActionString(szAction, action) || !buttonCode)
    return;   // no valid action, or an invalid buttoncode

  XBMCMod mods = XBMCKMOD_NONE;
  if (modifier)
    mods = TranslateKeyboardModifiers(modifier);

  // have a valid action, and a valid button - map it.
  // check to see if we've already got this (button,action) pair defined
  buttonMap::iterator it = map.find(pair<uint32_t, XBMCMod>(buttonCode, mods));
  if (it == map.end() || (*it).second.id != action || (*it).second.strID != szAction)
  {
    ...
    if (it != map.end())
      map.erase(it);
    CButtonAction buttonAction;
    buttonAction.id = action;
    buttonAction.strID = szAction;
    button aButton = pair<uint32_t, XBMCMod>(buttonCode, mods);
    map.insert(pair<button, CButtonAction>(aButton, buttonAction));
  }
}
...
void CButtonTranslator::MapWindowActions(TiXmlNode *pWindow, int windowID)
{
  ...
  { // map keyboard actions
    ...
    while (pButton)
    {
      ...
      const char *szModifier = pButton->Attribute("mod");
      if (pButton->FirstChild())
        MapAction(buttonCode, pButton->FirstChild()->Value(), map, szModifier);
      pButton = pButton->NextSiblingElement();
    }
  }
#if defined(HAS_SDL_JOYSTICK) || defined(HAS_EVENT_SERVER)
...
bool CButtonTranslator::TranslateActionString(const char *szAction, int &action)
{
  ...
  {
    if (strAction != "-")
      CLog::Log(LOGWARNING, "Keymapping error: no such action '%s' defined", strAction.c_str());
    return false;
  }
...
int CButtonTranslator::TranslateWindowString(const char *szWindow)
{
  ...
  if (StringUtils::IsNaturalNumber(strWindow))
  {
    ...
    if (iWindow > WINDOW_INVALID)
    {
      windowID = iWindow;
      CStdString strAppId;
      if (CAppManager::GetInstance().GetAppByWindowId(g_windowManager.GetActiveWindow(), strAppId))
        windowID = CAppManager::GetInstance().GetWindowId(strAppId, windowID);
    }
    else
      windowID = WINDOW_HOME + iWindow;
  }
// Boxee
  else if (strWindow.Equals("boxeebrowse")) windowID = WINDOW_BOXEE_BROWSE;
  else if (strWindow.Equals("boxeefeed")) windowID = WINDOW_BOXEE_FEED;
  else if (strWindow.Equals("boxeemain")) windowID = WINDOW_HOME;
  else if (strWindow.Equals("boxeemenu")) windowID = WINDOW_BOXEE_DIALOG_MAIN_MENU;
  else if (strWindow.Equals("boxeemediainfo")) windowID = WINDOW_BOXEE_MEDIA_INFO;
  else if (strWindow.Equals("boxeemediasourceaddfolder")) windowID = WINDOW_BOXEE_MEDIA_SOURCE_ADD_FOLDER;
  else if (strWindow.Equals("boxeemediasourceaddshare")) windowID = WINDOW_BOXEE_MEDIA_SOURCE_ADD_SHARE;
  else if (strWindow.Equals("boxeemediasource")) windowID = WINDOW_BOXEE_MEDIA_SOURCES;
  else if (strWindow.Equals("boxeemediasourceinfo")) windowID = WINDOW_BOXEE_MEDIA_SOURCE_INFO;
  else if (strWindow.Equals("boxeemediasourcelist")) windowID = WINDOW_BOXEE_MEDIA_SOURCE_LIST;
  else if (strWindow.Equals("boxeewizardaddsource")) windowID = WINDOW_BOXEE_WIZARD_ADD_SOURCE;
  else if (strWindow.Equals("boxeewizardaddsourcemanual")) windowID = WINDOW_BOXEE_WIZARD_ADD_SOURCE_MANUAL;
  else if (strWindow.Equals("boxeewizardaudio")) windowID = WINDOW_BOXEE_WIZARD_AUDIO;
  else if (strWindow.Equals("boxeewizardnetwork")) windowID = WINDOW_BOXEE_WIZARD_NETWORK;
  else if (strWindow.Equals("boxeewizardnetworkmanual")) windowID = WINDOW_BOXEE_WIZARD_NETWORK_MANUAL;
  else if (strWindow.Equals("boxeewizardsourcemanager")) windowID = WINDOW_BOXEE_WIZARD_SOURCE_MANAGER;
  else if (strWindow.Equals("boxeewizardsourcename")) windowID = WINDOW_BOXEE_WIZARD_SOURCE_NAME;
  else if (strWindow.Equals("boxeewizardtimezone")) windowID = WINDOW_BOXEE_WIZARD_TIMEZONE;
  else if (strWindow.Equals("boxeeuserinfo")) windowID = WINDOW_DIALOG_BOXEE_USER_INFO;
// end Boxee
  else if (strWindow.Equals("home")) windowID = WINDOW_HOME;
  else if (strWindow.Equals("programs")) windowID = WINDOW_PROGRAMS;
  else if (strWindow.Equals("pictures")) windowID = WINDOW_BOXEE_BROWSE_PHOTOS;
  ...
  else if (strWindow.Equals("music")) windowID = WINDOW_BOXEE_BROWSE_ALBUMS;
  ...
  else if (strWindow.Equals("video") || strWindow.Equals("videos")) windowID = WINDOW_BOXEE_BROWSE_TVSHOWS;
  ...
  else if (strWindow.Equals("osdsubtitlesettings")) windowID = WINDOW_DIALOG_SUBTITLE_OSD_SETTINGS;
