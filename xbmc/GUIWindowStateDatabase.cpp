...
{
  bool bRetVal = false;
  LOG(LOG_LEVEL_DEBUG, "GUIWindowStateDatabase::LoadState, iWindow=%d , iView=%d , iSortMethod=%d, iSelectedItem=%d , iSelectedCategory=%d ",iWindow,iView,strSortMethod.c_str(),iSelectedItem,strSelectedCategory.c_str());
  ...
  if (pDS)
  {
    if (pDS->num_rows() != 0)
    {
      LOG(LOG_LEVEL_DEBUG, "GUIWindowStateDatabase::LoadState, state found, idWindow='%d' , strCategory='%s'", iWindow, strSelectedCategory.c_str());
    ...
    {
      LOG(LOG_LEVEL_DEBUG, "GUIWindowStateDatabase::LoadState, state not found, idWindow='%d', strCategory='%s'", iWindow, strSelectedCategory.c_str());
    }
