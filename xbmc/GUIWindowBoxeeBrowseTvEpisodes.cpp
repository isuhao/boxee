  ...
  switch ( message.GetMessage() )
  {
    ...
    case GUI_MSG_CLICKED:
    {
      ...
      if (iControl == BUTTON_SUBSCRIBE_ON || iControl == BUTTON_SUBSCRIBE_OFF)
      {
        ...
        bSubscribed = ((CEpisodesWindowState*)m_windowState)->OnSubscribe();
...
bool CGUIWindowBoxeeBrowseTvEpisodes::OnBind(CGUIMessage& message)
{
  if (message.GetPointer() && message.GetControlId() == 0)
  {
    ...
    ExtractSeasons(items);

    //CONTROL_ENABLE_ON_CONDITION(SEASONS_LIST, (m_setSeasons.size() > 1));
  }
...
void CGUIWindowBoxeeBrowseTvEpisodes::ExtractSeasons(CFileItemList* episodeList)
{
  ...
  while (it != m_setSeasons.end())
  {
    int season = (int) *it;
    CStdString label;
    //Extras id is 0 for local and -1 for online
    if(season == 0 || season == EXTRAS_SEASON_ID)
...
void CGUIWindowBoxeeBrowseTvEpisodes::AddSeparators(CFileItemList &items)
{
  ...
  if (!bHasSeasons)
  {
    ...
    if (items.Size() > 0 && items.Get(0)->HasProperty("isseparator") && items.Get(0)->GetLabel() == g_localizeStrings.Get(20435))
    {
      items.Get(0)->SetLabel(g_localizeStrings.Get(20435));
    }
  }
}
