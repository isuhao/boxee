...
void CGUIWindowBoxeeMain::OnInitWindow()
{
  ...
  SET_CONTROL_FOCUS(BTN_LIST, 2 /* Shows */);
}
...
#ifdef HAS_DVB
bool CGUIWindowBoxeeMain::RunOnBoardingWizardIfNeeded()
{
  // Run the OTA wizard
  if (!g_guiSettings.GetBool("ota.run"))
  {
    if (!CBoxeeOTAConfigurationManager::GetInstance().RunWizard())
    {
      return false;
    }
  ...
  {
    CGUIDialogBoxeeLiveTvScan* pDialog = (CGUIDialogBoxeeLiveTvScan*) g_windowManager.GetWindow(WINDOW_OTA_SCANNING);
    pDialog->DoModal();
    return pDialog->RequestedGotToLiveTv();
  }
    ...
    case GUI_MSG_CLICKED:
    {
      ...
      else if (iAction == ACTION_SELECT_ITEM || iAction == ACTION_MOUSE_LEFT_CLICK)
      {
        switch(iControl)
        {
          ...
          case BTN_FILES:
          {
            g_windowManager.ActivateWindow(WINDOW_BOXEE_BROWSE_LOCAL,"boxeeui://files/?path=network://protocols");
          }
          break;
          case BTN_WEB:
          {
          ...
          case BTN_DVB:
          {
            if (RunOnBoardingWizardIfNeeded())
            {
