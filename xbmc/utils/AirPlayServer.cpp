...
int CAirPlayServer::CTCPClient::ProcessRequest(CStdString& responseHeader, CStdString& responseBody)
{
  ...
  else if (uri == "/play")
  {
    ...
    {
      std::vector<char> binVector;
      const char* bodyChr = m_httpParser->getBody();
      for (unsigned int i = 0; i < m_httpParser->getContentLength(); i++)
        binVector.push_back(bodyChr[i]);

      PList::Structure* plist = PList::Structure::FromBin(binVector);
      if (plist)
      {
        PList::Dictionary& dict = *((PList::Dictionary*) plist);

        if (dict[std::string("Start-Position")])
        {
          position = ((PList::Real*) dict[std::string("Start-Position")])->GetValue();
        }

        if (dict[std::string("Content-Location")])
        {
          location = ((PList::String*) dict[std::string("Content-Location")])->GetValue();
        }
      }
      else
      {
